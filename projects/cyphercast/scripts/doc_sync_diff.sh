...
usage() {
  cat <<'EOF'
Usage: doc_sync_diff.sh <path-to-cyphercast-repo> [relative-docs-path]

Compares Codex HQ docs under projects/cyphercast/docs with the downstream
cyphercast repository docs to ensure they are in sync.

Arguments:
  path-to-cyphercast-repo  Absolute or relative path to the downstream repo.
